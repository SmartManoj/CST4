...
class ViewInBrowserCommand(sublime_plugin.TextCommand):
	...
	def loadPluginSettings(self, defaultBrowser):
		...
		# print(result['browser'],1,defaultBrowser,settings.get("browser"))
		#
		...
		if not os.path.exists(result["baseCommand"]):
			result["baseCommand"] = self.getBaseCommand(selectedOs[platform].get(result["browser"].strip('64')), osName)
	...
	def run(self, edit, browser=None):
		...
		#
		if pluginSettings["baseCommand"] and 1:
